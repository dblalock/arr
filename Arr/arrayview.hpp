...
// template<int Attrs, int StaticStride=0, class SizeBounds=NoBounds>
...
//         AxisT::is_used, typename AxisT::SizeBounds>;
...
// template<int Rank, class T1, class T2>
// static inline auto static_dot(const T1& lhs, const T1& rhs)
//     -> decltype(lhs[0] + rhs[0])
// {
//     decltype(lhs[0] + rhs[0]) ret = 0;
...
// template<int Order> struct ArrayViewInfo { using order == };
...
struct ArrayView {
    ...
    // template<int NumIdxs> using any_idxs_t = std::array<IdxT, NumIdxs>;
    // using idxset_t = typename IdxSet<rank, IdxT>::type;
    // using idxset_t = typename std::array<rank, IdxT>::type;
    ...
    // DataT& at(const idxs_t& idxs) {
    //     auto offset = flatten_idxs(idxs);
    //     assert(offset < ) // TODO add at() as a bounds-checked operator[]
    ...
    // template<class IntT=IdxT, REQ_RANGE_ENCOMPASSES(IdxT, IntT)>
    // DataT& operator[](std::array<IntT, 1> idxs) {
    //     static_assert(rank == 1, "Attempted to index into ArrayView "
    //         "with only one index!");
    //     return _data[flatten_idxs(idxs)];
    // }
    // template<class IntT=IdxT, REQ_RANGE_ENCOMPASSES(IdxT, IntT)>
    // DataT& operator[](std::array<IntT, 2> idxs) {
    //     static_assert(rank == 2, "Attempted to index into ArrayView "
    //         "with two indices!");
    //     return _data[flatten_idxs(idxs)];
    // }
    // template<class IntT=IdxT, REQ_RANGE_ENCOMPASSES(IdxT, IntT)>
    // DataT& operator[](std::array<IntT, 3> idxs) {
    //     static_assert(rank == 3, "Attempted to index into ArrayView "
    //         "with three indices!");
    //     return _data[flatten_idxs(idxs)];
    // }
...
// template<int rank, REQ(rank == 1)>
// std::array<IdxT, rank> idxs_from_flat_idx(IdxT offset) {
//     return offset;
// }
// template<REQ(rank == 2)>
// std::array<IdxT, rank> idxs_from_flat_idx(IdxT offset) {
        ...
        StaticDim0, 0, 0, 0, IdxT, DataT>::type
{
    ...
    //      -also enforce that static sizes trump dynamic ones
...
// here are some examples of the API we want to support:
// make_view<RowMajor, -1, 16, -1>(data, 5, -1, 10)
// make_view(data, 10)
// make_view<ColMajor, 20>(data)
// make_view<NCHW>(data, 3)  // static assert fails; needs 4D
